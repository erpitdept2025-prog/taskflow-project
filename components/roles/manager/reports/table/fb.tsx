...
export const FBTable: React.FC<FBProps> = ({
  ...
  // companies state stays but initialized empty, no fetch anymore
  const [companies, setCompanies] = useState<Company[]>([]);

  const [activities, setActivities] = useState<FB[]>([]);
  const [loadingCompanies, setLoadingCompanies] = useState(false); // optional, can remove if you want
  const [loadingActivities, setLoadingActivities] = useState(false);
  const [errorCompanies, setErrorCompanies] = useState<string | null>(null); // optional, can remove if you want
  ...
  // Removed companies fetch useEffect
  ...
  // Merge company info into activities — companies is always empty, so fallback values will be used
  ...
  // Filter logic, pagination, totals, unique quotation counts unchanged...
  ...
  const filteredActivities = useMemo(() => {
    ...
    return mergedActivities
      .filter((item) =>
        ["facebook marketplace"].includes(item.source?.toLowerCase() ?? "")
      )
      .filter((item) => {
